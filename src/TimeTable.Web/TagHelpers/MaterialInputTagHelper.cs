...
namespace TimeTable.Web.TagHelpers {
	...
	[HtmlTargetElement("materialinput", Attributes = ForAttributeName, TagStructure = TagStructure.WithoutEndTag)]
	public class MaterialInputTagHelper : TagHelper {
		...
		public MaterialInputTagHelper(IHtmlGenerator generator) {
			Generator = generator;
		}
		...
		public override void Process(TagHelperContext context, TagHelperOutput output) {
			...
			if (metadata.AdditionalValues.ContainsKey("LabelMessage")) {
				labelMessage = metadata.AdditionalValues["LabelMessage"] as string;
			}

			var tagBuilderForLabel = Generator.GenerateLabel(
			   ViewContext,
			   For.ModelExplorer,
			   For.Name,
			...
			output.Content.SetHtmlContent(tagBuilderForLabel.InnerHtml);
